...
// ---------------------------------------------------------------------------
// Grounder / double-play logic (handleGrounder in hitBall.ts)
// ---------------------------------------------------------------------------

// Mock value guide for hitBall ground-ball path (balanced, non-power):
//   call 1: getRandomInt(1000) — needs >=750 to enter the pop-out/grounder block
//   call 2: getRandomInt(100)  — grounder check: <40 → grounder, >=40 → fly-out
//   call 3: getRandomInt(100)  — DP check (only when runner on 1st, <2 outs): <65 → DP

describe("grounder – simple ground out (no runner on 1st)", () => {
  it("records one out at first", () => {
    vi.spyOn(rngModule, "random")
      .mockReturnValueOnce(0.999) // randomNumber=999 >= 750 → pop-out area
      .mockReturnValueOnce(0.3);  // grounder check=30 < 40 → grounder
    const { state, logs } = dispatchAction(
      makeState({ baseLayout: [0, 0, 0], outs: 0 }),
      "hit", { hitType: Hit.Single, strategy: "balanced" }
    );
    expect(state.outs).toBe(1);
    expect(logs.some(l => /ground ball/i.test(l))).toBe(true);
    expect(logs.some(l => /out at first/i.test(l))).toBe(true);
  });

  it("runners already on base are preserved after ground out (no force)", () => {
    vi.spyOn(rngModule, "random")
      .mockReturnValueOnce(0.999)
      .mockReturnValueOnce(0.3);
    const { state } = dispatchAction(
      makeState({ baseLayout: [0, 1, 0], outs: 0 }),
      "hit", { hitType: Hit.Single, strategy: "balanced" }
    );
    expect(state.outs).toBe(1);
    expect(state.baseLayout[1]).toBe(1); // runner on 2nd stays
  });
});

describe("grounder – double play (runner on 1st, <2 outs)", () => {
  it("records two outs and removes runner from 1st", () => {
    vi.spyOn(rngModule, "random")
      .mockReturnValueOnce(0.999) // pop-out area
      .mockReturnValueOnce(0.3)   // grounder
      .mockReturnValueOnce(0.5);  // DP check=50 < 65 → DP
    const { state, logs } = dispatchAction(
      makeState({ baseLayout: [1, 0, 0], outs: 0 }),
      "hit", { hitType: Hit.Single, strategy: "balanced" }
    );
    expect(state.outs).toBe(2);
    expect(state.baseLayout[0]).toBe(0); // runner on 1st is out
    expect(logs.some(l => /double play/i.test(l))).toBe(true);
  });

  it("DP with 1 out ends the half-inning (3 total outs)", () => {
    vi.spyOn(rngModule, "random")
      .mockReturnValueOnce(0.999)
      .mockReturnValueOnce(0.3)
      .mockReturnValueOnce(0.5);
    const { state, logs } = dispatchAction(
      makeState({ baseLayout: [1, 0, 0], outs: 1, atBat: 0, inning: 2 }),
      "hit", { hitType: Hit.Single, strategy: "balanced" }
    );
    // 1+2 = 3 outs → nextHalfInning (outs reset to 0, atBat flips to 1)
    expect(state.outs).toBe(0);
    expect(state.atBat).toBe(1);
    expect(logs.some(l => /double play/i.test(l))).toBe(true);
  });

  it("DP preserves runners on 2nd and 3rd", () => {
    vi.spyOn(rngModule, "random")
      .mockReturnValueOnce(0.999)
      .mockReturnValueOnce(0.3)
      .mockReturnValueOnce(0.5);
    const { state } = dispatchAction(
      makeState({ baseLayout: [1, 1, 1], outs: 0 }),
      "hit", { hitType: Hit.Single, strategy: "balanced" }
    );
    expect(state.outs).toBe(2);
    expect(state.baseLayout[0]).toBe(0); // runner from 1st out
    expect(state.baseLayout[1]).toBe(1); // runner on 2nd stays
    expect(state.baseLayout[2]).toBe(1); // runner on 3rd stays
  });
});

describe("grounder – fielder's choice (runner on 1st, DP not turned)", () => {
  it("records one out; batter reaches 1st safely", () => {
    vi.spyOn(rngModule, "random")
      .mockReturnValueOnce(0.999) // pop-out area
      .mockReturnValueOnce(0.3)   // grounder
      .mockReturnValueOnce(0.7);  // DP check=70 >= 65 → no DP → fielder's choice
    const { state, logs } = dispatchAction(
      makeState({ baseLayout: [1, 0, 0], outs: 0 }),
      "hit", { hitType: Hit.Single, strategy: "balanced" }
    );
    expect(state.outs).toBe(1);
    expect(state.baseLayout[0]).toBe(1); // batter safely at 1st
    expect(logs.some(l => /fielder's choice/i.test(l))).toBe(true);
  });

  it("fielder's choice preserves runners on 2nd and 3rd", () => {
    vi.spyOn(rngModule, "random")
      .mockReturnValueOnce(0.999)
      .mockReturnValueOnce(0.3)
    ...
    expect(state.outs).toBe(1);
    expect(state.baseLayout[0]).toBe(1); // batter at 1st
    expect(state.baseLayout[1]).toBe(1); // runner on 2nd stays
    expect(state.baseLayout[2]).toBe(1); // runner on 3rd stays
  });
});

describe("grounder – no DP attempt with 2 outs", () => {
  it("simple ground out even with runner on 1st when 2 outs", () => {
    vi.spyOn(rngModule, "random")
      .mockReturnValueOnce(0.999)
      .mockReturnValueOnce(0.3); // grounder
    const { state, logs } = dispatchAction(
      makeState({ baseLayout: [1, 0, 0], outs: 2 }),
      "hit", { hitType: Hit.Single, strategy: "balanced" }
    );
    // 3rd out → half-inning transition
    expect(state.outs).toBe(0);
    expect(logs.some(l => /ground ball/i.test(l))).toBe(true);
    expect(logs.some(l => /double play/i.test(l))).toBe(false);
  });
});

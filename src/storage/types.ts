 ...
 *     stable roster ID (same ID generated by generateRoster for that team).
 *   - Substituted players are recorded under their own playerKey, not the replaced slot.
 *   - Future trade/free-agent work can evolve playerKey to a global player ID without
 *     breaking existing history rows (old rows keep their original playerKey).
 */
export interface PlayerGameStatDoc {
   ...
   * v1 format: `${teamId}:${playerId}` for both custom and stock players.
   */
  playerKey: string;

...
### Visual snapshots
Playwright visual snapshot baselines must be regenerated in an environment that matches the CI Docker container (`mcr.microsoft.com/playwright:v1.58.2-noble`). Font and system library differences between environments produce false diffs. Use the container locally or trigger `.github/workflows/update-visual-snapshots.yml`.
